	...
	<div class="post_edit_delete">
		<c:choose>
			<c:when test="${logininfo.memId eq detaildto.memId || logininfo.memId eq 'admin'}">
				<form id="revieweditform" action="revieweditform"> 
		...
		<ul class="pagination">
			<c:choose>
				<c:when test="${currentCpage == 1}">
					<li class="page-item disabled"><a class="page-link"
						href="/reviewpostdetail?boardId=${bId}&page=${currentCpage - 1}"
						tabindex="-1" aria-disabled="true">이전</a></li>
				</c:when>
				<c:otherwise>
					<li class="page-item active"><a class="page-link" id="nextPageLink"
						href="/reviewpostdetail?boardId=${bId}&page=${currentCpage - 1}">이전</a></li>
				</c:otherwise>
			</c:choose>
				...
				<c:choose>
					<c:when test="${i == currentCpage}">
						<li class="page-item activeNumber"><a class="page-link" id="nextPageLink"
							href="/reviewpostdetail?boardId=${bId}&page=${i}">${i}</a></li>
					</c:when>
					<c:otherwise>
			...
			<c:choose>
				<c:when test="${totalPage == currentCpage}">
						...
						href="/reviewpostdetail?boardId=${bId}&page=${currentCpage + 1}">다음</a></li>
				</c:when>
				<c:otherwise>
...
function deleteBoard(s, a) {
	if (!confirm("게시물을 삭제하시겠습니까?")) {
        ...
        alert("게시물이 삭제되었습니다.");
        location.href="/reviewdeleteBoard?boardId=" + s + "&memId=" + a;
    }
}
function deleteComment(bc,b, m) {
	if (!confirm("댓글을 삭제하시겠습니까?")) {
        ...
        alert("댓글이 삭제되었습니다.");
        location.href="/reviewdeleteComment?bcId=" + bc + " &boardId=" + b + "&memId=" + m;
    }
}
...
function deleteReply(bc,b, m) {
	if (!confirm("답글을 삭제하시겠습니까?")) {
        ...
        alert("답글이 삭제되었습니다.");
        location.href="/reviewdeleteReply?bcId=" + bc + " &boardId=" + b + "&memId=" + m;
    }
}
